(function() {
    ...
    window.CanvasSlideshow = function( options ) {
        ...
        options.navDit          = options.hasOwnProperty('navDit')  ?  options.navElement : document.querySelectorAll( '.slide-dit' );
            ...
            navItem.onclick = function( event ) {
                ...
                if ( this.getAttribute('data-nav') === 'next' ) {
                    ...
                    if ( that.currentIndex >= 0 && that.currentIndex < slideImages.length - 1 ) {
                        ...
                        $(".menu_nav ul a").eq(that.currentIndex+1).addClass("active").siblings().removeClass("active");
                    } else {
                        that.moveSlider( 0 );
                        $(".menu_nav ul a").eq(0).addClass("active").siblings().removeClass("active");
                    }
                    ...
                    if ( that.currentIndex > 0 && that.currentIndex < slideImages.length ) {
                        ...
                        $(".menu_nav ul a").eq(spriteImages.length-1).addClass("active").siblings().removeClass("active");
                    }
        ...
        $('.menu_nav ul a').click(function(){
            about_swiper.slideToLoop($(this).index())
            $('.menu_nav ul a').addClass('ban')
            setTimeout(()=>{
                $('.menu_nav ul a').removeClass('ban')
            },3000)
            clearInterval(timer);
            $(this).addClass('active').siblings().removeClass('active');
            that.moveSlider( $(this).index() );
        });
        ...
        function handle(delta) {
            if (delta <0){
                ...
                about_swiper.slideToLoop($(".menu_nav ul a.active").index())
            }else{
                if ( !isPlaying ) {
                    if ( that.currentIndex > 0 && that.currentIndex < slideImages.length ) {
                        that.moveSlider( that.currentIndex - 1 );
